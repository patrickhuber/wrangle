	...
	Describe("Execute", func() {
		...
		Context("WhenAllEnvVarsSet", func() {
			...
			It("should render both env vars", func() {
				...
				expected := fmt.Sprintf("%s=\n%s=%s\n%s=%s\n", global.BinPathKey, global.PackagePathKey, packagesPath, global.ConfigFileKey, configFilePath)
		...
		Context("WhenOnlyPackagePathSet", func() {
			...
			It("should render package path and not config file", func() {
				...
				Expect(err).To(BeNil())
				buffer := console.Out().(*bytes.Buffer)
				expected := fmt.Sprintf("%s=\n%s=%s\n%s=\n", global.BinPathKey, global.PackagePathKey, packagesPath, global.ConfigFileKey)
				Expect(buffer.String()).To(Equal(expected))
			})
		})
		Context("WhenOnlyConfigFileSet", func() {
			...
			It("should render package path and not config file", func() {
				...
				Expect(err).To(BeNil())
				buffer := console.Out().(*bytes.Buffer)
				expected := fmt.Sprintf("%s=\n%s=\n%s=%s\n", global.BinPathKey, global.PackagePathKey, global.ConfigFileKey, configFilePath)
				Expect(buffer.String()).To(Equal(expected))
			})
		})
